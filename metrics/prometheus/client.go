...
var (
	...
	clientCompleted = prometheus.NewCounterVec(
		...
		[]string{"name", "host", "path", "method", "status"},
	)
...
func (t *clientTracker) RequestStarted() {
	clientStarted.WithLabelValues(t.name, t.host, t.path, t.method).Inc()
}

func (t *clientTracker) RequestRead(duration time.Duration, size int) {
	if t.opts.sizes {
		clientRequestSize.WithLabelValues(t.name, t.host, t.path, t.method).Observe(float64(size))
	}
}

func (t *clientTracker) ResponseStarted(duration time.Duration, code int, header http.Header) {
	status := strconv.Itoa(code)
	clientCompleted.WithLabelValues(t.name, t.host, t.path, t.method, status).Inc()
	if t.opts.latency {
		clientLatency.WithLabelValues(t.name, t.host, t.path, t.method, status).Observe(duration.Seconds())
	}
}

func (t *clientTracker) ResponseDone(duration time.Duration, code int, size int) {
	if t.opts.sizes {
		clientResponseSize.WithLabelValues(t.name, t.host, t.path, t.method, strconv.Itoa(code)).Observe(float64(size))
	}
}
